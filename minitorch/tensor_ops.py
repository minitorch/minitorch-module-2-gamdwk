...
def tensor_map(fn: Callable[[float], float]) -> Any:
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _map(
        ...
        for s in out_shape:
            out_sizes *= s
        in_index = np.ndarray(shape=(len(in_shape),))
        out_index = np.ndarray(shape=(len(out_shape),))
        for i in range(out_sizes):
...
def tensor_zip(fn: Callable[[float, float], float]) -> Any:
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _zip(
        ...
        for s in out_shape:
            out_sizes *= s
        a_index = np.ndarray(shape=(len(a_shape),))
        b_index = np.ndarray(shape=(len(b_shape),))
        out_index = np.ndarray(shape=(len(out_shape),))
        for i in range(out_sizes):
            to_index(i, out_shape, out_index)
